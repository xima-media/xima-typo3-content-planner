/**
* Module: @xima/ximatypo3contentplanner/comment-todo-toggle
*
* Makes to-do checkboxes inside a comment's rendered content clickable directly from the
* display state (CP-30, #389), instead of requiring the comment to be opened for editing first.
* The RTE's saved to-do markup renders checkboxes as `disabled` outside the editor (see
...
* CommentTodoController::toggleTodoAction() - a DataHandler-routed write, same as any other
* comment edit.
*
...
class CommentTodoToggle {
  ...
  initEventListeners() {
    document.querySelectorAll('[data-todo-editable="true"]').forEach(container => {
      ...
      // string. A checkbox hand-authored outside .todo-list (the RTE's source editing toolbar)
