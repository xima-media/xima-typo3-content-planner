...
use Xima\XimaTypo3ContentPlanner\EventListener\Watcher\AutoWatchOnAssigneeChangedListener;
use Xima\XimaTypo3ContentPlanner\Service\WatcherService;

/**
 * AutoWatchOnAssigneeChangedListenerTest.
 *
 * @author Konrad Michalik <hej@konradmichalik.dev>
 * @license GPL-2.0-or-later
 */
final class AutoWatchOnAssigneeChangedListenerTest extends TestCase
{
    #[Test]
    public function watchesTheNewlyAssignedUser(): void
    {
        ...
        $watcherService->expects(self::once())
            ->method('watch')
            ->with('pages', 5, 9, WatchSource::Assignment);

        (new AutoWatchOnAssigneeChangedListener($watcherService))(
            new AssigneeChangedEvent('pages', 5, null, 9, 1),
        );
    }

    #[Test]
    public function doesNothingWhenAssigneeWasCleared(): void
    {
        $watcherService = $this->createMock(WatcherService::class);
        $watcherService->expects(self::never())->method('watch');

        (new AutoWatchOnAssigneeChangedListener($watcherService))(
            new AssigneeChangedEvent('pages', 5, 9, null, 1),
        );
    }
    ...
    public function doesNothingWhenTableIsNotWatchable(): void
    {
        ...
        (new AutoWatchOnAssigneeChangedListener($watcherService))(
            new AssigneeChangedEvent('sys_file_metadata', 5, null, 9, 1),
        );
    }
}
